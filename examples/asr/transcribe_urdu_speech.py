...
python transcribe_urdu_speech.py \
    model_path=null \
...
def main(cfg: TranscriptionConfig):
    ...
    with autocast():
        with torch.no_grad():
            # print(filepaths)
            transcriptions = asr_model.transcribe(paths2audio_files=[filepaths], batch_size=cfg.batch_size)
            print(transcriptions)
    logging.info(f"Finished transcribing {len(filepaths)} files !")
    logging.info(f"Transcription {transcriptions} ")
